...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "http://192.168.1.124:8082"}}, supports_credentials=True)
...
def distribute_project():
    ...
    if not project_id or not target_ip:
        return jsonify({'error': '缺少 project_id 或 target_ip'}), 400

    try:
        # 解析 ip:port
        if ':' not in target_ip:
            return jsonify({'error': 'target_ip 格式错误'}), 400
        ...
        logging.info(f"正在下发项目 {project_id} 到 {url}")
        response = requests.post(url, json=payload, timeout=10)

        if response.status_code == 200:
            logging.info(f"下发成功: {target_ip}")
            return jsonify({'message': f'下发成功至 {target_ip}'}), 200
        else:
            error_msg = response.text or "未知错误"
            logging.error(f"下发失败，状态码 {response.status_code}: {error_msg}")
            return jsonify({'error': f'下发失败: {error_msg}'}), 500

    except ValueError as e:
        logging.error(f"端口解析失败: {str(e)}")
        return jsonify({'error': '端口必须是数字'}), 400
    except requests.exceptions.ConnectionError:
        logging.error(f"无法连接学生端: {ip}:{port}")
        return jsonify({'error': '学生端未启动或网络不通'}), 500
    except requests.exceptions.Timeout:
        logging.error(f"下发超时: {ip}:{port}")
        return jsonify({'error': '下发超时'}), 500
    except Exception as e:
        logging.error(f"下发异常: {str(e)}", exc_info=True)
        return jsonify({'error': f'下发失败: {str(e)}'}), 500
...
def recycle_project(project_id):
    try:
        ...
        return jsonify({'message': '已通知学生端回收实验'}), 200
    except Exception as e:
        import traceback
